...
namespace CdZ.Repositorio.EF
{
    ...
    public class CavaleiroRepositorioEF : ICavaleiroRepositorio
    {
        ...
        public IEnumerable<Cavaleiro> Paginado(int qtdPorPagina, int pagina)
        {
            using (var db = new ContextoDeDados())
            {                
                return db.Cavaleiro.Include(_ => _.Imagens).Include(_ => _.Golpes).Include(_ => _.LocalNascimento).Include(_ => _.LocalTreinamento).OrderBy(_ => _.Nome).Skip(pagina * qtdPorPagina).Take(qtdPorPagina).ToList();
            }
        }
